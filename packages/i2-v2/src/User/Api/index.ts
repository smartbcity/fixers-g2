...
export const useGetUser = <T extends User = User>(
  params: QueryParams<{ id: string }, { item: T }>
) => {
  const requestProps = useAuthenticatedRequest('im')
...
export const useUpdateUser = <T extends User = User>(
  params?: CommandParams<T, { id: string }>
) => {
  const requestProps = useAuthenticatedRequest('im')
...
export const useUserUpdatePassword = (
  params?: CommandParams<UserUpdatePasswordCommand, UserUpdatePasswordResult>
) => {
  const requestProps = useAuthenticatedRequest('im')
...
export const useUserResetPassword = (
  params?: CommandParams<UserResetPasswordCommand, UserUpdatePasswordResult>
) => {
  const requestProps = useAuthenticatedRequest('im')
...
export const useUserUpdateEmail = (
  params?: CommandParams<UserUpdateEmailCommand, UserUpdatedEmailEvent>
) => {
  const requestProps = useAuthenticatedRequest('im')
